...
// paleta de colores
const TRANSITABLE: Rgb<u8> = Rgb([255, 255, 255]); // blanco
const BLOQUEADO: Rgb<u8>   = Rgb([160, 160, 160]); // gris
const OPEN_COLOR: Rgb<u8>  = Rgb([144, 238, 144]); // verde claro
const CLOSE_COLOR: Rgb<u8> = Rgb([0, 100, 0]);     // verde oscuro
const START_COLOR: Rgb<u8> = Rgb([0, 0, 255]);     // azul
const GOAL_COLOR: Rgb<u8>  = Rgb([255, 0, 0]);     // rojo
const PATH_COLOR: Rgb<u8>  = Rgb([255, 200, 0]);   // amarillo fuerte
const GRID_COLOR: Rgb<u8>  = Rgb([210, 210, 210]); // gris clarito para la grilla
...
pub fn visualize_final_state(
    ...
    // 3. open (verde claro)
    for (_, coord) in open.iter() {
        fill_cell(&mut img, *coord, OPEN_COLOR);
    }
...
fn draw_thick_path(img: &mut RgbImage, path: &[Coords], scale: u32, thickness: u32, color: Rgb<u8>) {
    ...
    for window in path.windows(2) {
        ...
        let dx = p2.0 - p1.0;
        let dy = p2.1 - p1.1;
        let len = (dx * dx + dy * dy).sqrt().max(0.001); // evita división por cero
        let perp_x = -dy / len;
        let perp_y = dx / len;
